...
class TWCommand( chunks.Command ) :

	def __init__( self, cmd ) :
		self.cmd_id = cmd.cmd_id
		self.time_code = cmd.time_code
		self.player_id = cmd.player_id
		self.payload = cmd.payload

	# decode, if I'm a build order command
	def decode( self ) :
		pass
...
class TWChunk( chunks.Chunk ) :
	...
	def decode_commands( self ) :
		for i, cmd in enumerate( self.commands ) :
			if self.is_bo_cmd( cmd ) :
				cmd = TWCommand( cmd ) # "type cast" it.
				self.commands[ i ] = cmd
				cmd.decode()
